...
#our %EXPORT_TAGS = ( 'all' => [ qw(makewebpivot) ] );
our %EXPORT_TAGS;

our @EXPORT_OK = qw(makewebpivot);

our @EXPORT = qw( );

sub import {
	print STDERR "@_\n";
	Spreadsheet::WriteExcel::WebPivot->export_to_level(1, @_);
}
...
sub getDataTypes {
	...
	foreach my $key (@keys) {
		...
		switch ($href->{$key}) {
			case /^\d+$/	{ $type = q(type="int"); 
						$typename = 'int'; }
			case /^\d+\.\d+$/	{ $type = q(type="float");
							$typename = 'float'; }
			case qr/^\d{4}\-\d{2}\-\d{2}/	{ $type = q(type="dateTime"); $typename = 'dateTime'; }
			else		{ $type = q(maxLength="255"); 
						$typename = 'text'; }
		}
...
#-----------------------------------------------------------------------
# this function setups of the subdirectory required by Excel's web object
#
sub makepivotdir {
	my $file = shift;
	my $title = shift;
...
sub addPivotData {
	...
	if( 'Sum' eq $self->{'SummaryType'} ) {
		$self->{items}->{$href->{$key1}} += $href->{$keyN};
		$self->{grandTotal} += $href->{$keyN};
	} else {
		# for the Count Summary type. No other summary types supported
		$self->{items}->{$href->{$key1}}++;
		$self->{grandTotal}++;
	}
...
sub makewebpivot {
	...
	$dbh = FakeDBI->new($dbh)	if( ref($dbh) eq 'ARRAY' );

	# get the location of the templates directory.
	#my $templdir = '/../templates/excel';
	#$self->{TemplateDir} = $templdir;
	...
	makepivotdir($file,$title,$rquerykeys,'Count');	
	#if( ! -d $file . "_files");

	my $fh = FileHandle->new(">$file"."_files/$file".'_4267_cachedata001.xml');
	...
	#$self->makeSummaryPage($rquerykeys,$file);	#if($summarypage);
...
<xml xmlns:o="urn:schemas-microsoft-com:office:office">
 ...
 <o:File HRef="CACHENAME_4267_cachedata001.xml"
...
<!----------------------------->
<div id="CACHENAME_4267" align=left x:publishsource="Excel"><object
 id="CACHENAME_4267_PivotTable"
 ...
 &lt;CacheFile HRef=&quot;CACHENAME_files/CACHENAME_4267_cachedata001.xml&quot;/&gt;&#13;&#10;
 &lt;RefreshDate&gt;2003-06-17T17:50:29&lt;/RefreshDate&gt;&#13;&#10;
 &lt;RefreshDateCopy&gt;2003-06-17T17:50:29&lt;/RefreshDateCopy&gt;&#13;&#10;
 &lt;/PTSource&gt;&#13;&#10;  
 &lt;Name&gt; TITLE &lt;/Name&gt;&#13;&#10;
 &lt;DataMember&gt;XLDataSource&lt;/DataMember&gt;&#13;&#10;
...
sub new {
	...
	if( defined($arg) and ref($arg) eq 'ARRAY' ) {
		bless($arg); # I don't ever expect anyone to inherit from this
	} else { 
		$arg = [];
		bless($arg);
	}
...
my $xml = SpreadSheet::WriteExcel::WebPivot->new();
...
my $xml = Spreadsheet::WriteExcel::WebPivot->new();
$xml->makewebpivot(\@array, '', \@fields, 'Count', 'exceltest',
'Test Pivot');

=head1 AUTHOR

Nathan Lewis, E<lt>nathanlewis42@yahoo.co.uk<gt>
